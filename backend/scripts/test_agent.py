...
Runs a clearly-simple ticket end-to-end, then an ambiguous ticket that must
pause at human_approval; resumes it once with 'reject' and once with 'approve'.
...
def run_simple(graph) -> None:
    ...
    show("SIMPLE ticket -> auto-resolved", result)


def run_ambiguous(graph, decision: str, expected_status: str) -> None:
    tid = str(uuid.uuid4())
    result = graph.invoke(
        {
            "ticket_text": (
    ...
    assert result.get("__interrupt__"), "expected an interrupt"
    show(f"AMBIGUOUS ticket -> paused (will {decision})", payload)

    resumed = graph.invoke(Command(resume=decision), config=_config(tid))
...
def main() -> int:
    ...
    graph = build_graph()
    run_simple(graph)
    run_ambiguous(graph, "reject", "escalated")
    run_ambiguous(graph, "approve", "resolved")
